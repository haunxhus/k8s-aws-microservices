...
if [ -n "${TARGET_BRANCH}" ] || [ "$GIT_PRE_COMMIT" != "$GIT_CURR_COMMIT" ]; then
  ...
  if [ -z "$CHANGED_FILE_FOLDER" ]; then
    ...
    for filePath in ${CHANGED_FILE_FOLDER[@]}; do
      ...
      if [[ $isIgnore -eq 1 ]]; then
        for ((ind = 0; ind < ${#LIST_PROJECT_FOLDER[@]}; ind++)); do
      ...
      for ((pos = 0; pos < ${listK8sConfigLength}; pos++)); do
    ...
    for key in ${!FOLDER_NEED_TO_CHECK[@]}; do
    ...
    for value in ${FOLDER_NEED_TO_CHECK[@]}; do
